...
#                 bar_posterior=False):
    ...
    def _nice_step(x):
        # returns 1, 2, or 5 times a power of 10 near x
        if x <= 0:
            return 1.0
    ...
    def _nice_panel_ticks(ymax, max_major=4, headroom=0.05):
        ymax = float(max(ymax, 1e-9))
        ymax_padded = ymax * (1.0 + headroom)
        # choose step so we have <= max_major ticks above 0
        step = _nice_step(ymax_padded / max_major)
        top_tick = step * math.ceil(ymax_padded / step)
        ticks = np.arange(0.0, top_tick + 1e-12, step)
        return top_tick, ticks
        """
        Given a raw ymax, return a slightly padded ymax and a set of major ticks.
        n_major ~ how many major ticks to aim for (excl. 0 if pruned).
        """
        ymax = float(ymax)
        if ymax <= 0:
            ymax = 1.0

        # small headroom so tallest peak doesn't clip
        ymax_padded = ymax * (1.0 + headroom)

        # choose a nice step ~ order-of-magnitude of ymax/n_major
        step = 10 ** math.floor(math.log10(ymax_padded / max(n_major, 1)))
        # if step is too coarse, refine (helps when ymax is small)
        if ymax_padded / step > (n_major * 2):
            step *= 2
        elif ymax_padded / step < (n_major - 1):
            step /= 2.0

        top_tick = step * math.ceil(ymax_padded / step)
        ticks = np.arange(0.0, top_tick + 1e-9, step)
    ...
    for sample_num, curr_ax in enumerate(plotted_axes):
        ...
        print("New debug active")

        # EDIT
        curr_ax.relim(); curr_ax.autoscale_view()
        _, raw_ymax = curr_ax.get_ylim()

        panel_ymax, panel_ticks = _nice_panel_ticks(raw_ymax, max_major=4, headroom=0.05)
        panel_fake_ymin = -0.6 * panel_ymax                 # keep the little negative headroom for arcs

        # set limits first
        curr_ax.set_ylim(panel_fake_ymin, panel_ymax)

        if showYaxis:
            # apply sparse per-panel ticks and labels
            curr_ax.set_yticks(panel_ticks)
            curr_ax.set_yticklabels(
                [("" if t <= 0 else (f"{t:.1f}" if (t % 1) != 0 else f"{int(t)}")) for t in panel_ticks],
                fontsize=font_size
            )

            # make the LEFT spine match the visible tick range (avoid too-long/too-short look)
            if len(panel_ticks) > 0:
                curr_ax.spines["left"].set_bounds(panel_ticks[0], panel_ticks[-1])
            else:
                curr_ax.spines["left"].set_bounds(0, panel_ymax)

            curr_ax.yaxis.set_ticks_position('left')
            curr_ax.spines["right"].set_color('none')

            # give labels a little room
            curr_ax.tick_params(axis='y', pad=6)
        else:
            curr_ax.spines["left"].set_color('none')
            curr_ax.spines["right"].set_color('none')
            curr_ax.set_yticks([])
            curr_ax.yaxis.set_major_locator(NullLocator())
        ...
        if len(panel_ticks) >= 2:
            halfway_ypos = (panel_ticks[-1] - panel_ticks[-2]) / 2.
            label_ypos = panel_ticks[-2] + halfway_ypos
        else:
            label_ypos = panel_ticks[-1]
